...
- Configuración de columna: `st.column_config`
    ...
    Es una característica útil para configurar cómo se muestran y se utilizan los datos. Está diseñada para usarse con la opción `column_config` tanto en `st.dataframe` como en `st.data_editor`. 
    Los métodos de esta clase permiten personalizar las columnas de datos para diferentes tipos de datos, como texto, números, listas, URLs, imágenes y más. 
    La configuración de columnas brinda la flexibilidad de visualizar e interactuar con los datos de la manera que prefiera. Los siguientes son los métodos de clase de `st.column_config`:
